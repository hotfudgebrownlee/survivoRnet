...
comp_detes <- comparisons %>%
  ...
  select(cast_id, inspo_id, count, past5, gender) %>% unique() %>% ungroup()
```
...
``` {r link_tree}
link_tree <- comparisons %>% filter(!(inspo_id %in% unlinked$inspo_id))
```

``` {r prepare_network}
edges <- link_tree %>% select(cast_id, inspo_id) %>%
  rename(from = cast_id, to = inspo_id)

nSize <- link_tree %>% select(inspo_id, inspo_name) %>%
...
nodes <- link_tree %>%
  ...
  mutate(type = 1:(nrow(link_tree)*4), type = case_when(
    type %% 2 == 0 ~ type / 2,
